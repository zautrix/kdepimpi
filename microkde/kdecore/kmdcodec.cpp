...
void KCodecs::base64Decode( const QByteArray& in, QByteArray& out )
{
    ...
    if ( len > 1 )
    {
      while (didx < len-2)
      {
          out[didx] = (((out.at(sidx) << 2) & 255) | ((out.at(sidx+1) >> 4) & 003));
          out[didx+1] = (((out.at(sidx+1) << 4) & 255) | ((out.at(sidx+2) >> 2) & 017));
          out[didx+2] = (((out.at(sidx+2) << 6) & 255) | (out.at(sidx+3) & 077));
          sidx += 4;
          didx += 3;
      }
    }
    ...
    if (++didx < len )
        out[didx] = (((out.at(sidx+1) << 4) & 255) | ((out.at(sidx+2) >> 2) & 017));
